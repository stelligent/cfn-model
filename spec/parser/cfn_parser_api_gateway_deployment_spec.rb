...
describe CfnParser do
  ...
  context 'API Gateway Deployment is referenced in an API stage with a Usage Plan. ' do
    it 'returns API Gateway Deployment with a usage plan' do
      expected_api_deployments = api_deployment_refd_in_stage_with_one_usage_plan(cfn_model: CfnModel.new)
      test_templates('api_gateway/api_gateway_deployment_refd_in_stage_with_usage_plan').each do |test_template|
        cfn_model = @cfn_parser.parse IO.read(test_template)
        api_deployments = cfn_model.resources_by_type 'AWS::ApiGateway::Deployment'
