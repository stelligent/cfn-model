...
describe CfnParser do
  ...
  context 'API Gateway Stag with no Usage Plan. ' do
    it 'returns an api stage with no usage plan' do
      expected_api_stage = api_stage_with_no_usage_plan(cfn_model: CfnModel.new)
  ...
  context 'One API Gateway Stag with one Usage Plan. ' do
    it 'returns API Gateway Stag with one Usage Plan' do
  ...
  context 'Two API Gateway Stages - each associated with a different Usage Plan. ' do
    it 'Two API Gateway Stages - each associated with a different Usage Plan' do
      expected_api_stage_1, expected_api_stage_2 = twp_api_stages_each_with_different_usage_plan(cfn_model: CfnModel.new)
