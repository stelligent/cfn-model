...
describe 'wildcard_patterns', :rule do
  ...
  context 'Passing "both" pattern option' do
    it 'returns patterns with * at front and back' do
      expect(wildcard_patterns('test', pattern_types: ['both'])).to eq \
        %w[test *test* *tes* *te* *t* *est* *es* *e* *st* *s* *t* *]
    end
  end

  context 'Passing "wrong" pattern option' do
    it 'raises a "no pattern of type: wrong" exception' do
      # Parenthesis need to change to curly braces when using a 'raise_error' matcher
      # https://stackoverflow.com/a/34387063
      expect { wildcard_patterns('test', pattern_types: ['wrong']) }.to \
        raise_error(RuntimeError, /no pattern of type: wrong/)
    end
  end

  context 'Passing "front" and "back" pattern_types options' do
    it 'returns patterns with * at front and * at back' do
      expect(wildcard_patterns('test', pattern_types: %w[front back])).to eq \
        %w[test *test *est *st *t test* tes* te* t* *]
    end
  end

  context 'Passing no pattern_types option' do
    it 'returns all patterns( front, back, both)' do
      expect(wildcard_patterns('test')).to eq \
        %w[test *test *est *st *t test* tes* te* t* *test* *tes* *te* *t* *est* *es* *e* *st* *s*
           *t* *]
    end
  end
  context 'Passing an integer as the input "string"' do
    it 'returns valid results' do
      expect(wildcard_patterns(123)).to eq \
        %w[123 *123 *23 *3 123* 12* 1* *123* *12* *1* *23* *2* *3* *]
    end
  end
end
